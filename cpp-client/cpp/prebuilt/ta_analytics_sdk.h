...
#pragma once
#if defined(_WIN32)
#define WILDFIREDATA_API __declspec(dllexport)
#endif
...
#if defined(_WIN32)
class WILDFIREDATA_API WildfireAnalyticsAPI;
#else
class WildfireAnalyticsAPI;
#endif
class TDConfig;
class TAHttpSend;
class TDJSONObject;
...
public:
    ...
    /**
     * WildfireAnalyticsAPI Destructor
     */
    static void UnInit();
    ...
    */
    ...
    ~WildfireAnalyticsAPI();

    static WildfireAnalyticsAPI* instance_;

private:
    /**
     * After the SDK initialization is complete, the saved instance can be obtained through this api
     */
    WildfireAnalyticsAPI(const string &server_url,
                         const string &appid);

    WildfireAnalyticsAPI(const WildfireAnalyticsAPI &);

    //WildfireAnalyticsAPI &operator=(const WildfireAnalyticsAPI &);
...
public:
    ...
    string mExtraId;
    WildfireAnalyticsEvent(const string &eventName, const TDJSONObject &properties);
};
class TDFirstEvent: public WildfireAnalyticsEvent
{
public:
...
class TDUpdatableEvent: public WildfireAnalyticsEvent
{
public:
    TDUpdatableEvent(const string & eventName,const TDJSONObject & properties, const string & eventId);
};
class TDOverWritableEvent: public WildfireAnalyticsEvent
{
public:
