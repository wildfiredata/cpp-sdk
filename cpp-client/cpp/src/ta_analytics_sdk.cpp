...
WildfireAnalyticsEvent::WildfireAnalyticsEvent(const string &eventName, const TDJSONObject& properties) {
    this->mEventName = eventName;
    this->mProperties = properties;
}
TDFirstEvent::TDFirstEvent(const string & eventName, const TDJSONObject & properties):WildfireAnalyticsEvent(eventName,properties) {
    this->mType = FIRST;
    this->mExtraId = "";
}
...
TDUpdatableEvent::TDUpdatableEvent(const string & eventName, const TDJSONObject & properties, const string & eventId):WildfireAnalyticsEvent(eventName,properties) {
    this->mExtraId = eventId;
    this->mType = UPDATABLE;
}
TDOverWritableEvent::TDOverWritableEvent(const string & eventName, const TDJSONObject & properties, const string & eventId):WildfireAnalyticsEvent(eventName,properties) {
    this->mExtraId = eventId;
...
WildfireAnalyticsAPI *WildfireAnalyticsAPI::instance_ = NULL;

void WildfireAnalyticsAPI::fetchRemoteConfigCallback(bool calibrateTime,bool encrypt) {
    if(instance_ && instance_->httpSend_){
        Response res = instance_->httpSend_->fetchRemoteConfig();
        if(res.code_ == 200){
            ta_cpp_helper::printSDKLog("[WildfireData] Info: Get remote config success, "+res.body_);
            try{
                ...
                if (root_obj != NULL && root_obj->type == tacJSON_Object) {
                    ...
                    if(calibrateTime){
                        ...
                        WildfireAnalyticsAPI::CalibrateTime(timeStamp);
                    }
                    if(encrypt){
...
bool WildfireAnalyticsAPI::Init(TDConfig &config) {
    if (!instance_) {
        bool initSdkSuccess = true;
        string server_url = config.server_url;
        string appid = config.appid;
        if (server_url.empty()) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: serverUrl can not be empty");
            return false;
        }

        if (appid.empty()) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: appId can not be empty");
            return false;
        }
        ...
        if(config.dataExpiration > 10){
            ta_cpp_helper::data_expiration = config.dataExpiration*24*60*60;
        }

        TATaskQueue* dataTaskQue = new (std::nothrow) TATaskQueue();
        if (dataTaskQue == nullptr) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error:  Failed to allocate memory for dataTaskQue");
            initSdkSuccess = false;
        }


        WildfireAnalyticsAPI* ins = new (std::nothrow) WildfireAnalyticsAPI(server_url, appid);
        if (ins == nullptr) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: Failed to allocate memory for WildfireAnalyticsAPI Init");
            return false;
        }
        ...
        if (sqlite == nullptr || !initStatus) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: Failed to allocate memory for TASqliteDataQueue Init");
            initSdkSuccess = false;
        ...
        if (httpSend == nullptr) {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: Failed to allocate memory for TAHttpSend Init");
            initSdkSuccess = false;
        }

        TDSystemInfo* tdSystemInfo = new (std::nothrow) TDSystemInfo();
        if(tdSystemInfo == nullptr){
            ta_cpp_helper::printSDKLog("[WildfireData] Error: Failed to allocate memory for TDTimeCalibrated Init");
            initSdkSuccess = false;
        }
        ...
        if(config.mode == TDMode::TD_NORMAL) {
            ...
            if (dataTaskQue == nullptr) {
                ta_cpp_helper::printSDKLog("[WildfireEngine] Failed to allocate memory for networkTaskQue");
                initSdkSuccess = false;
            }
            ...
            if(ftask == nullptr){
                ta_cpp_helper::printSDKLog("[WildfireData] Error: Failed to allocate memory for TDFlushTask Init");
                initSdkSuccess = false;
            }
        ...
        if(initSdkSuccess){
            ...
            t.detach();
            string result = "[WildfireData] Info: WildfireData SDK initialize success, AppId: = " + appid + ", ServerUrl = " + server_url + ", DeviceId = " + ta_cpp_helper::getDeviceID()+ ", LibVersion  = " + TD_LIB_VERSION;
...
bool WildfireAnalyticsAPI::Init(const string &server_url,
                                const string &appid) {
...
void WildfireAnalyticsAPI::CalibrateTime(int64_t &timestamp) {
    if(instance_){
        instance_->tdSystemInfo->enableTimeCalibrated(timestamp);
    }
}

void WildfireAnalyticsAPI::EnableLog(bool enable) {
    if(enable){
        TAEnableLog::setTALogType(LOGCONSOLE);
    }
}

void WildfireAnalyticsAPI::EnableLogType(TALogType type) {
    TAEnableLog::setTALogType(type);
}

void WildfireAnalyticsAPI::AddUser(string eventType, const TDJSONObject &properties)
{
    AddEvent(eventType, "", properties, "", "");
}


bool WildfireAnalyticsAPI::AddEvent(const string &action_type,
    ...
    if(instance_->mode == TDMode::TD_NORMAL){
        ...
        if (sqiteInsetTask == nullptr) {
            ta_cpp_helper::printSDKLog("[WildfireEngine] Failed to allocate memory for TASqiteInsetTask Init");
            return false;
        }
        ...
        if (debugTask == nullptr) {
            ta_cpp_helper::printSDKLog("[WildfireEngine] Failed to allocate memory for TADebugTask Init");
            return false;
        }
...
void WildfireAnalyticsAPI::InnerFlush() {
    TAFlushTask *_flushTask = new TAFlushTask(m_sqlite, httpSend_, appid_);
    if (_flushTask == nullptr) {
        ta_cpp_helper::printSDKLog("[WildfireEngine] Failed to allocate memory for TAFlushTask Init");
        return ;
    }
...
void WildfireAnalyticsAPI::Flush() {
    if (instance_) {
        if(instance_->mode == TDMode::TD_NORMAL){
...
void WildfireAnalyticsAPI::registerTECallback(void (*p)(int, const string&)) {
    if (instance_) {
        instance_->funcs.push_back(p);
    }
}

void WildfireAnalyticsAPI::SetDynamicSuperProperties(DynamicSuperProperties dynamicSuperProperties) {
    if(instance_){
...
vector<void(*)(int,const string&)> WildfireAnalyticsAPI::getTECallback(){
    if (instance_) {
...
void WildfireAnalyticsAPI::Track(const string &event_name, const TDJSONObject &properties) {
    if (instance_) {
...
void WildfireAnalyticsAPI::Track(TDFirstEvent* event) {
    if (instance_) {
...
void WildfireAnalyticsAPI::Track(TDUpdatableEvent* event) {
    if (instance_) {
...
void WildfireAnalyticsAPI::Track(TDOverWritableEvent* event) {
    if (instance_) {
...
void WildfireAnalyticsAPI::Track(const string &event_name) {
    TDJSONObject properties_node;
    Track(event_name, properties_node);
}


void WildfireAnalyticsAPI::Login(const string &login_id) {
    if (instance_) {
        ta_account_mtx.lock();
        ...
        ta_account_mtx.unlock();
        ta_cpp_helper::printSDKLog("[WildfireData] Info: Login SDK, AccountId = "+login_id);
    }
}

void WildfireAnalyticsAPI::LogOut() {
    if (instance_) {
        ta_account_mtx.lock();
        string login_id = "";
        instance_->account_id_ = login_id;
        ...
        ta_cpp_helper::updateAccount(instance_->appid_.c_str(), login_id.c_str(), path);
        ta_account_mtx.unlock();
        ta_cpp_helper::printSDKLog("[WildfireData] Info: Logout SDK");
    }
}

void WildfireAnalyticsAPI::Identify(const string &distinct_id) {
    if (instance_) {
        ta_distinct_mtx.lock();
        ...
        ta_distinct_mtx.unlock();
        ta_cpp_helper::printSDKLog("[WildfireData] Info: Setting distinct ID, DistinctId = "+distinct_id);
    }
}

void WildfireAnalyticsAPI::SetSuperProperty(const TDJSONObject &properties){
    if (instance_) {
...
void WildfireAnalyticsAPI::GetSuperProperties(TDJSONObject &properties) {
    if(instance_){
        ta_superProperty_mtx.lock();
...
void WildfireAnalyticsAPI::GetPresetProperties(TDJSONObject &properties) {
    if (instance_ && instance_->tdSystemInfo) {
        properties = instance_->tdSystemInfo->presetProperties;
    }
}

void WildfireAnalyticsAPI::ClearSuperProperty(){
    if (instance_) {
        ta_superProperty_mtx.lock();
...
void WildfireAnalyticsAPI::UnsetSuperProperties(const string &propertyName) {
    if (instance_) {
        ta_superProperty_mtx.lock();
...
string WildfireAnalyticsAPI::DistinctID() {
    return instance_ ? instance_->distinct_id_ :  "";
}

string WildfireAnalyticsAPI::GetDeviceId() {
    return instance_ ? instance_->device_id_ :  "";
}

void WildfireAnalyticsAPI::TimeEvent(const string &event_name) {
    if(instance_){
        ta_timer_mtx.lock();
...
void WildfireAnalyticsAPI::UserSet(const TDJSONObject &properties) {
    if (instance_) {
        instance_->AddUser(TD_EVENT_TYPE_USER_SET, properties);
    }
    
}
void WildfireAnalyticsAPI::UserSetOnce(const TDJSONObject &properties) {
    if (instance_) {
        instance_->AddUser(TD_EVENT_TYPE_USER_SETONCE, properties);
    }
    
}
void WildfireAnalyticsAPI::UserAdd(const TDJSONObject &properties) {
    if (instance_) {
        instance_->AddUser(TD_EVENT_TYPE_USER_ADD, properties);
    }
    
}
void WildfireAnalyticsAPI::UserAppend(const TDJSONObject &properties) {
    if (instance_) {
...
void WildfireAnalyticsAPI::UserUniqAppend(const TDJSONObject &properties) {
    if (instance_) {
        instance_->AddUser(TD_EVENT_TYPE_USER_UNIQ_APPEND, properties);
    }
}

void WildfireAnalyticsAPI::UserDelete() {
    if (instance_) {
        TDJSONObject  node;
...
void WildfireAnalyticsAPI::UserUnset(string propertyName)  {
    if (instance_) {
        TDJSONObject  obj;
...
string WildfireAnalyticsAPI::StagingFilePath() {
    return instance_ ? instance_->staging_file_path_ :  "";
}

WildfireAnalyticsAPI::WildfireAnalyticsAPI(const string& server_url, const string& appid): server_url_(server_url), appid_(appid) {}

WildfireAnalyticsAPI::~WildfireAnalyticsAPI() {}

void WildfireAnalyticsAPI::UnInit()
{
