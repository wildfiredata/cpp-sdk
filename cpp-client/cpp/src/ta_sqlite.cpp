...
namespace wildfiredata {
    ...
    TASqliteDataQueue::TASqliteDataQueue(std::string appid,bool &initStatus,bool enableCrypt,int v,string &pKey,string &dbPath): m_appid(appid), m_allmessagecount(0) {
        ...
        if (openDBStatus)
        {
            ta_cpp_helper::printSDKLog("[WildfireData] Error: Can't open database: ");
            ta_cpp_helper::printSDKLog(sqlite3_errmsg(ta_database));
            initStatus = false;
        }
        else
        {
            ta_cpp_helper::printSDKLog("[WildfireData] Info: Opened database successfully ");
            initStatus = true;
        }
        ...
        try
        {
            ...
            {
                ta_cpp_helper::printSDKLog("[WildfireData] Error: SQL error:");
            ...
            {
                ta_cpp_helper::printSDKLog("[WildfireData] Info: Table created successfully");
                initStatus = true;
                // allmessageCount
...
//        vector<tuple<string, string>> records;
    ...
    void TASqliteDataQueue::removeExpirationData(){
        if (isStop) {
            return;
        }
        sqlite3_stmt* stmt = NULL;
        try
        {
            timeb t;
            ftime(&t);
            int64_t time = t.time - ta_cpp_helper::data_expiration;
