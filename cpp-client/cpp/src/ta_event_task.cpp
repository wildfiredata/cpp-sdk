...
namespace wildfiredata {
    ...
    void TASqiteInsetTask::DoTask() {
        ta_cpp_helper::printSDKLog("[WildfireData] Info: Enqueue data, "+m_event);
        ...
        if (messageCount >= ta_cpp_helper::flush_bulk_size) {
            TANetworkTask *task = new TANetworkTask(m_sqliteQueue, m_httpSend, m_appid);
            if (task == nullptr) {
                ta_cpp_helper::printSDKLog("[WildfireEngine] Failed to allocate memory for TANetworkTask Init");
                return ;
            }
    ...
    void TAFlushTask::DoTask() {
        ta_cpp_helper::printSDKLog("[WildfireData] Info: Flush Task");
