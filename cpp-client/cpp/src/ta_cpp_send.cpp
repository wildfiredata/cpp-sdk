...
namespace wildfiredata {
    ...
    bool HttpSender::send(const string &data) {
        ...
        if (response.code_ != 200) {
            ta_cpp_helper::printSDKLog("WildfireAnalytics SDK send failed: ");
            ta_cpp_helper::printSDKLog(response.body_);
            ta_cpp_helper::handleTECallback(1003,response.body_);
            return false;
        }else{
            ta_cpp_helper::printSDKLog("[WildfireData] Debug: Send event, Request = "+data);
            ta_cpp_helper::printSDKLog("[WildfireData] Debug: Send event, Response = "+response.body_);
        }
        return true;
    }
    ...
    bool HttpSender::sendDebugData(const string &debugData) {
        ...
        if (response.code_ != 200) {
            ...
            ta_cpp_helper::printSDKLog("[WildfireData] Debug: Send event, Request = "+debugData);
            ta_cpp_helper::printSDKLog("[WildfireData] Debug: Send event, Response = "+response.body_);
        }
        return true;
    }
    ...
    Response HttpSender::fetchRemoteConfig() {
        ...
        if (response.code_ != 200) {
            ta_cpp_helper::printSDKLog("WildfireAnalytics SDK fetch remote config: ");
            ta_cpp_helper::printSDKLog(response.body_);
            ta_cpp_helper::handleTECallback(1003,response.body_);
        }
        return response;
...
//        if (send_result) {
//            ta_cpp_helper::printSDKLog("[WildfireData] Debug: Send event, Request = "+json_record);
//        }
